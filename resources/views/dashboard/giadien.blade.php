...
@section('content')
    <div class="container-fluid px-4">
        ...
        <div class="card mb-4">
            <div class="card-header">
                ...
                Bảng giá điện hiện tại (Version: {{ $latestVersion->id ?? 'N/A' }} - Áp dụng từ:
                {{ $latestVersion ? \Carbon\Carbon::parse($latestVersion->ngayapdung)->format('d/m/Y') : 'N/A' }})
            </div>
            <div class="card-body">
                    ...
                    <tbody>
                        @forelse($bacgia as $bg)
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-primary btn-sm" title="Sửa" data-bs-toggle="modal"
                                        data-bs-target="#editModal{{ $bg->mabac }}">
                                    ...
                                    <form id="delete-form-{{ $bg->mabac }}" action="{{ route('giadien.destroy', $bg->mabac) }}"
    ...
    @foreach($bacgia as $bg)
        <div class="modal fade" id="editModal{{ $bg->mabac }}" tabindex="-1" aria-labelledby="editModalLabel{{ $bg->mabac }}"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" name="densokw"
                                    value="{{ $bg->densokw == 99999 ? '' : $bg->densokw }}" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Đơn giá (VNĐ/kWh)</label>
                                <input type="number" step="0.01" class="form-control" name="dongia" value="{{ $bg->dongia }}"
                                    required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Ghi chú version</label>
